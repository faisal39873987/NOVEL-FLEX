...
async function uploadNovelCover(client, userId, file) {
  ...
  if (!allowedTypes.has(file.type)) {
    throw new Error("غلاف الرواية يجب أن يكون JPG أو PNG أو WebP.");
  }
  if (file.size > 2 * 1024 * 1024) {
    throw new Error("حجم غلاف الرواية يجب ألا يتجاوز 2MB.");
  }
...
async function saveAuthorNovel(form) {
  ...
  if (!manualCoverUrl && !existingCoverUrl && (!coverFile || !coverFile.size)) {
    appState.authorPortal.error = "غلاف الرواية مطلوب. ارفع صورة بنسبة 2:3 ويفضل 1200x1800.";
    route();
    return;
  }
    ...
    `
      ...
      <form class="editor-form" data-author-novel-form data-novel-mode="${mode}" data-book-id="${escapeHtml(activeBook.id || "")}">
        ...
        <label>
          ...
          <small>النسبة المطلوبة 2:3، المقاس الموصى به 1200x1800، والحد الأقصى 2MB.</small>
        </label>
        <label>
