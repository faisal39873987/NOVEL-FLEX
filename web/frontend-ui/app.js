...
const academyPosts = [
  {
    title: "افتتاحية تشد القارئ",
    type: "دليل كتابة",
    time: "٨ دقائق",
    summary: "ابدأ بمشهد واضح، سؤال درامي، ووعد سردي يعرف القارئ لماذا يكمل.",
  },
  {
    title: "مراجعة الفصل قبل النشر",
    type: "قائمة تدقيق",
    time: "٥ دقائق",
    summary: "راجع العنوان، ترتيب الأحداث، الأخطاء اللغوية، ونهاية الفصل قبل الضغط على نشر.",
  },
  {
    title: "إيقاع السلسلة",
    type: "تخطيط",
    time: "٦ دقائق",
    summary: "وازن بين الفصول القصيرة والطويلة، واجعل كل فصل يضيف قرارا أو كشفا جديدا.",
  },
];

const academyChecklist = [
  "العنوان واضح ومناسب لمحتوى الفصل.",
  "الفصل يحتوي نصا أو ملفا صالحا للقراءة.",
  "الغلاف لا يقطع النصوص المهمة على الجوال.",
  "الحالة مضبوطة: مسودة للمراجعة أو منشور للقراء.",
];
...
const appState = {
  ...
  reading: {
    ...
    recordedBookViews: new Set(),
    recordedChapterReads: new Set(),
  },
  profileMetrics: {
    loaded: false,
    loading: false,
...
function adminTableLabel(table) {
  const labels = {
    profiles: "المستخدمون",
    writer_profiles: "الكتّاب",
    books: "الروايات",
    ratings: "المراجعات",
    reports: "البلاغات",
...
function adminBookStatusLabel(status) {
  const labels = {
    draft: "مسودة",
...
function adminRoleLabel(role) {
  ...
  return labels[role] || role || "قارئ";
}

function shortAdminId(value) {
  const text = String(value || "");
  return text ? text.slice(0, 8) : "غير متاح";
}

function adminDate(value) {
  return value ? new Date(value).toLocaleDateString("ar") : "بدون تاريخ";
}
...
function adminReportStatusLabel(status) {
  ...
  return labels[status] || status || "مفتوح";
}

function adminReportTarget(report = {}) {
  if (report.book_id) return `رواية ${shortAdminId(report.book_id)}`;
  if (report.chapter_id) return `فصل ${shortAdminId(report.chapter_id)}`;
  if (report.comment_id) return `تعليق ${shortAdminId(report.comment_id)}`;
  if (report.review_id || report.rating_id) return `تقييم ${shortAdminId(report.review_id || report.rating_id)}`;
  if (report.reported_user_id) return `مستخدم ${shortAdminId(report.reported_user_id)}`;
  return report.target_type || report.type || "عنصر غير محدد";
}

function adminReportDetails(report = {}) {
  return String(report.details || report.description || report.message || report.body || "").trim();
}
...
async function loadAdminPortal({ force = false } = {}) {
  ...
  try {
    ...
    if (!isAdminUser()) {
      appState.adminPortal.error = "حسابك الحالي لا يملك صلاحية مدير.";
      appState.adminPortal.loaded = true;
      return;
    }
...
function updateAdminCategory(form) {
  ...
  const slug = String(formData.get("slug") || "")
    ...
    .replace(/\s+/g, "-");
  if (!name || !slug) {
    appState.adminPortal.message = "اسم التصنيف والمعرّف مطلوبان.";
    route();
    return;
  }
  return runAdminAction(
...
function normalizeBook(row = {}) {
  ...
  return {
    ...
    viewsCount: Number(row.views_count || row.reads_count || 0),
    likesCount: Number(row.likes_count || row.favorites_count || 0),
...
function normalizeChapter(row = {}) {
  ...
  return {
    ...
    content:
      ...
      row.description_ar ||
      row.description ||
      "محتوى الفصل الكامل سيظهر هنا عند اكتمال نشر النص.",
...
function localSearchBooks(query) {
  const needle = normalizeSearchText(query);
  if (!needle) return [];
  return readerBooks().filter((book) => {
    const haystack = normalizeSearchText([book.title, book.author, book.genre, book.summary, ...(book.tags || [])].join(" "));
...
async function countUserRows(client, table, column, userId) {
...
async function loadProfileMetrics({ force = false } = {}) {
  const user = appState.auth.user;
  if (!user) {
    resetProfileMetrics();
    appState.profileMetrics.loaded = true;
    return;
  }
  if (appState.profileMetrics.loading) return;
  if (appState.profileMetrics.loaded && !force) return;
  ...
  if (!client) {
    appState.profileMetrics = {
      ...appState.profileMetrics,
      loaded: true,
      error: "Supabase client غير جاهز.",
    };
    route();
    return;
  }

  appState.profileMetrics.loading = true;
  appState.profileMetrics.error = "";
  route();
  ...
  try {
    ...
    appState.profileMetrics = {
      ...
      loaded: true,
      loading: false,
      error: error.message || "تعذر تحميل أرقام الحساب.",
    };
  } finally {
    route();
  }
...
async function saveReadingProgress(bookId, chapterId, progress = 65) {
  ...
  try {
    const existing = await client.from("reading_progress").select("id").eq("user_id", user.id).eq("book_id", bookId).limit(1);
...
async function recordBookView(bookId) {
  const client = supabaseClient();
  if (!client || !isUuid(bookId) || appState.reading.recordedBookViews.has(bookId)) return;
  appState.reading.recordedBookViews.add(bookId);
  try {
    const { error } = await client.from("book_view_events").insert({
      user_id: appState.auth.user?.id || null,
      book_id: bookId,
    });
    if (error) throw error;
  } catch (_) {
    appState.reading.recordedBookViews.delete(bookId);
  }
}

async function recordChapterRead(bookId, chapterId, eventType = "opened") {
  const client = supabaseClient();
  const key = `${bookId}:${chapterId}:${eventType}`;
  if (!client || !isUuid(bookId) || !isUuid(chapterId) || appState.reading.recordedChapterReads.has(key)) return;
  appState.reading.recordedChapterReads.add(key);
  try {
    ...
    appState.reading.recordedChapterReads.delete(key);
  }
}
...
async function loadSocialFeatures(book, { force = false } = {}) {
  ...
  if (!isUuid(book.id)) {
    ...
    appState.social.reviewsByBook.set(book.id, []);
    return;
  }
...
async function searchReaderBooks(query) {
  ...
  const fallbackResults = localSearchBooks(needle);
  if (!appState.reader.books.length) {
    appState.reader.searchResults.set(needle, fallbackResults);
    route();
    return;
  }
  const client = supabaseClient();
  ...
  try {
    ...
    const { data, error } = await client
      ...
      .limit(50);
    if (error) throw error;
    const liveResults = (data || []).map(normalizeBook);
    appState.reader.searchResults.set(needle, liveResults.length ? liveResults : fallbackResults);
  } catch (error) {
...
async function loadReaderChapters(bookId) {
  ...
  if (!isUuid(bookId)) {
    appState.reader.chaptersByBook.set(
      bookId,
      chapters.map((chapter) => ({ ...chapter, bookId })),
    );
    route();
    return;
  }
  ...
  try {
    const chaptersResult = await client
      .from("chapters")
      .select("id,book_id,chapter_number,title_ar,title_en,content_text,file_path,audio_path,cover_url,status,published_at,created_at,updated_at")
      .eq("book_id", bookId)
      .eq("status", "published")
...
function resetAuthorPortal() {
  ...
  appState.authorPortal.loadError = "";
...
function safeStorageFileName(file) {
  const rawName = file?.name || "cover.jpg";
  return rawName.replace(/[^A-Za-z0-9._-]/g, "_");
}

function jpegFileName(file) {
  const base = safeStorageFileName(file).replace(/\.[^.]+$/, "");
...
function setAuthorCoverUploadStatus(message, progress = 0, active = false) {
  appState.authorPortal.coverUpload = {
    message,
...
function userFacingAuthorError(error, fallback = "تعذر تنفيذ العملية. حاول مرة أخرى.") {
  const message = error?.message || "";
  if (/bucket|storage/i.test(message)) return "تعذر رفع الغلاف الآن. تحقق من إعدادات التخزين ثم حاول مرة أخرى.";
  if (/network|fetch|failed to fetch/i.test(message)) return "تعذر الاتصال بالخادم. تحقق من الشبكة ثم حاول مرة أخرى.";
  if (/permission|policy|row-level|rls|unauthorized|jwt/i.test(message)) return "لا تملك الصلاحية الكافية لتنفيذ هذا الإجراء.";
  return message || fallback;
}

async function logAuthorPortalException(client, error, context = {}) {
  console.error("Author portal error", { error, context });
  if (!client) return;
  try {
    ...
    console.warn("Author portal Supabase log failed", logError);
  }
}

function validateAuthorCoverFile(file) {
  if (!file || !file.size) return;
  const extension = (file.name || "").split(".").pop()?.toLowerCase() || "";
  const allowedExtension = ["jpg", "jpeg", "png", "webp"].includes(extension);
  if (!AUTHOR_COVER_TYPES.has(file.type) && !allowedExtension) {
    throw new Error("صيغة الغلاف غير مدعومة. الصيغ المقبولة: JPG أو JPEG أو PNG أو WebP.");
  }
}
...
function canvasToJpegBlob(canvas, quality = AUTHOR_COVER_JPEG_QUALITY) {
  return new Promise((resolve, reject) => {
    canvas.toBlob(
      (blob) => {
        if (blob) resolve(blob);
        else reject(new Error("تعذر ضغط صورة الغلاف."));
      },
      "image/jpeg",
      quality,
    );
...
async function optimizeAuthorCoverImage(file) {
...
async function uploadNovelCover(client, userId, file) {
  if (!file || !file.size) return "";
  ...
  setAuthorCoverUploadStatus("جار رفع الغلاف إلى التخزين...", 80, true);
  route();

  const fileName = `${Date.now()}_${jpegFileName(optimizedFile)}`;
  const path = `${userId}/books/${fileName}`;
  const { error } = await client.storage.from("book-covers").upload(path, optimizedFile, {
    cacheControl: "3600",
...
async function uploadChapterCover(client, userId, bookId, chapterNumber, file) {
  ...
  if (!allowedTypes.has(file.type)) {
    throw new Error("غلاف الفصل يجب أن يكون JPG أو PNG أو WebP.");
  }
  if (file.size > 2 * 1024 * 1024) {
    throw new Error("حجم غلاف الفصل يجب ألا يتجاوز 2MB.");
  }

  const fileName = `${Date.now()}_${safeStorageFileName(file)}`;
  const path = `${userId}/books/${bookId}/chapters/${chapterNumber}/${fileName}`;
  const { error } = await client.storage.from("chapter-covers").upload(path, file, {
    cacheControl: "3600",
    contentType: file.type,
    upsert: false,
  });
  if (error) throw error;
  const { data } = client.storage.from("chapter-covers").getPublicUrl(path);
  return data?.publicUrl || "";
}
...
async function loadAuthorChapters(bookId, { force = false } = {}) {
  ...
  try {
    const { data, error } = await client
      .from("chapters")
      .select("id,book_id,chapter_number,title_ar,title_en,content_text,file_path,audio_path,cover_url,status,created_at,updated_at,published_at")
      .eq("book_id", bookId)
      .order("chapter_number", { ascending: true });
...
async function saveAuthorChapter(form) {
  ...
  const payload = {
    ...
    published_at: statusAction === "publish" ? keepPublishedAt || new Date().toISOString() : null,
    updated_at: new Date().toISOString(),
  };
  ...
  if (statusAction === "publish" && !payload.content_text && !payload.file_path) {
    appState.authorPortal.error = "نص الفصل أو ملف الفصل مطلوب عند النشر. يمكنك حفظه كمسودة الآن وإكماله لاحقًا.";
    route();
    return;
  }
  ...
  try {
    const uploadedCoverUrl = coverFile?.size ? await uploadChapterCover(client, user.id, bookId, payload.chapter_number, coverFile) : "";
    ...
    appState.authorPortal.error = error.message || "تعذر حفظ الفصل.";
  } finally {
    appState.authorPortal.saving = false;
    route();
  }
...
async function loadAuthorPortal({ force = false } = {}) {
  ...
  try {
    ...
    await logAuthorPortalException(client, error, {
      action: "load_author_portal",
      user_id: user.id,
    });
    appState.authorPortal.loadError = userFacingAuthorError(
      error,
      "تعذر تحميل بوابة الكاتب الآن. تحقق من الاتصال أو الصلاحيات ثم حاول مرة أخرى.",
    );
    appState.authorPortal.loaded = true;
  } finally {
    appState.authorPortal.loading = false;
...
async function saveAuthorNovel(form, requestedStatusAction = "") {
  ...
  const payload = {
    ...
    status: statusAction === "publish" ? "published" : "draft",
    language: "ar",
    published_at: statusAction === "publish" ? new Date().toISOString() : null,
    updated_at: new Date().toISOString(),
  };
  ...
  try {
    ...
    await logAuthorPortalException(client, error, {
      action: "save_author_novel",
      mode,
      book_id: bookId || null,
      has_cover_file: Boolean(coverFile?.size),
      cover_size: coverFile?.size || 0,
    });
    appState.authorPortal.error = userFacingAuthorError(error, "تعذر حفظ الرواية. راجع بيانات الرواية والغلاف ثم حاول مرة أخرى.");
  } finally {
    appState.authorPortal.saving = false;
    if (!appState.authorPortal.error && appState.authorPortal.coverUpload.progress >= 100) {
      setAuthorCoverUploadStatus(appState.authorPortal.coverUpload.message, 100, false);
    }
    route();
  }
}
...
function browseHref(activeGenre = "", filter = "all", sort = "popular") {
  const path = activeGenre ? `/genre/${activeGenre}` : "/browse";
...
function FilterTabs(activeGenre = "", activeFilter = "all", activeSort = "popular") {
  const filters = [
    ["all", dual("الكل", "All")],
    ["completed", dual("مكتملة", "Completed")],
    ["ongoing", dual("مستمرة", "Ongoing")],
  ];
  return `
    <div class="tabs">
      ${filters.map(([value, label]) => `<a class="${activeFilter === value ? "is-active" : ""}" href="${browseHref(activeGenre, value, activeSort)}">${label}</a>`).join("")}
    </div>
  `;
}

function SortTabs(activeGenre = "", activeFilter = "all", activeSort = "popular") {
  const sorts = [
    ["popular", dual("الأشهر", "Popular")],
    ["rating", dual("التقييم", "Rating")],
    ["chapters", dual("الأطول", "Longest")],
    ["updated", dual("آخر تحديث", "Time updated")],
  ];
  return `
    <div class="sort-tabs">
      ${sorts.map(([value, label]) => `<a class="${activeSort === value ? "is-active" : ""}" href="${browseHref(activeGenre, activeFilter, value)}">${label}</a>`).join("")}
    </div>
  `;
}
...
function NovelHero(book) {
  ...
  const isLoadingChapters = appState.reader.loadingChapters.has(book.id);
  ...
  const readAction = isLoadingChapters
    ? `<button class="btn primary" type="button" disabled>جاري تحميل الفصول...</button>`
    : firstChapter
      ? `<a class="btn primary" href="${startHref}">اقرأ</a>`
      : `<button class="btn primary" type="button" disabled>${chapterError ? "تعذر تحميل الفصول" : "لا توجد فصول منشورة"}</button>`;
  return `
    <section class="novel-hero">
      ...
      <div class="novel-hero-copy">
        ...
        <div class="action-row">
          ...
          ${book.authorId ? `<button class="btn ghost" data-follow-author="${escapeHtml(book.authorId)}">${following ? "متابع" : "متابعة الكاتب"}</button>` : ""}
        </div>
      </div>
    </section>
  `;
...
function SocialPanel(book) {
  if (!isUuid(book.id)) {
    return `<div class="social-panel"><div class="toast">هذه رواية تجريبية للعرض. التفاعل والمراجعات تعمل مع روايات Supabase الحقيقية.</div></div>`;
  }
...
function ReviewForm(book) {
  if (!isUuid(book.id)) {
    return EmptyState("المراجعات غير متاحة للبيانات التجريبية", "ستعمل المراجعات تلقائيا مع الروايات الحقيقية عند نشر المحتوى.", "/browse");
  }
  return `
  ...
  return PageShell(
    ...
    `
      <section class="home-hero">
        ...
        ${featured ? BookCard(featured) : EmptyState("لا توجد روايات منشورة", "ستظهر الروايات هنا فور نشر أول عمل جاهز للقراءة.", "/browse")}
      </section>
      ${Section(
        "بوابات المنصة",
      ...
      ${Section("الأكثر قراءة", "مختارات جاهزة للقراءة.", liveBooks.length ? `<div class="book-grid">${liveBooks.slice(0, 4).map(BookCard).join("")}</div>` : EmptyState("لا توجد روايات منشورة", "ستظهر المختارات بعد توفر روايات منشورة.", "/browse"), `<a class="btn ghost" href="#/browse">عرض الكل</a>`)}
      ${Section("تصنيفات سريعة", "اختصر الطريق إلى النوع الذي تحبه.", liveGenres.length ? `<div class="genre-cloud">${liveGenres.map((genre) => `<a href="#/genre/${genre.slug}">${escapeHtml(genre.name)}<small>${genre.count}</small></a>`).join("")}</div>` : EmptyState("لا توجد تصنيفات", "ستظهر التصنيفات بعد تفعيلها للمستخدمين.", "/browse"))}
    `,
  );
}
      ...
      `
        <div class="browse-layout">
          ...
          <section class="browse-results">
            ${EmptyState("التصنيف غير موجود", "اختر تصنيفًا من القائمة أو عد إلى كل الروايات.", "/browse")}
          </section>
        </div>
      `,
    );
  }
  const filteredByGenre = activeGenre ? liveBooks.filter((book) => book.genreSlug === activeGenre) : liveBooks;
  const filteredByStatus = filteredByGenre.filter((book) => {
    if (activeFilter === "completed") return book.status === "مكتملة" || book.rawStatus === "completed";
    if (activeFilter === "ongoing") return book.status !== "مكتملة" && book.rawStatus !== "completed";
    return true;
  });
  const list = [...filteredByStatus].sort((a, b) => {
    if (activeSort === "rating") return Number(b.rating || 0) - Number(a.rating || 0);
    if (activeSort === "chapters") return Number(b.chapters || 0) - Number(a.chapters || 0);
    if (activeSort === "updated") return String(b.publishedAt || b.updated || "").localeCompare(String(a.publishedAt || a.updated || ""));
    return 0;
  });
  return PageShell(
    "/browse",
    `
      <div class="browse-layout">
        ${GenreSidebar(activeGenre)}
        <section class="browse-results">
          <div class="page-title"><span class="eyebrow">${dual("تصفح", "Browse")}</span><h1>${genre ? genre.name : dual("تصنيفات الروايات", "Genre of Novels")}</h1><p>${genre ? "استكشف الروايات ضمن هذا التصنيف." : "مختارات روايات جاهزة للقراءة والتصفية."}</p></div>
          ${FilterTabs(activeGenre, activeFilter, activeSort)}
          ${SortTabs(activeGenre, activeFilter, activeSort)}
          ${list.length ? `<div class="book-grid">${list.map(BookCard).join("")}</div>` : EmptyState("لا توجد روايات مطابقة", "جرّب فلترًا آخر أو عد إلى كل الروايات.", "/browse")}
        </section>
      </div>
    `,
  );
...
function renderSearch(query = "") {
  ...
  return PageShell(
    ...
    `
      ...
      ${Section(normalized ? `نتائج "${normalized}"` : "اقتراحات شائعة", "نتائج مطابقة للروايات المتاحة.", isLoading ? LoadingState() : searchError ? EmptyState("تعذر تنفيذ البحث", searchError, "/browse") : results.length ? `<div class="list-stack">${results.map(BookListItem).join("")}</div>` : EmptyState("لا توجد نتائج", "جرّب كلمة أقصر أو تصنيفا مختلفا.", "/browse"))}
    `,
  );
}
...
function renderNovelDetails(bookId) {
  ...
  const isLiveBook = isUuid(book.id);
  const reviewsBody = isLiveBook
    ? `${ReviewForm(book)}${appState.social.loadingBooks.has(book.id) ? LoadingState() : reviewError ? `<p class="auth-alert error">${escapeHtml(reviewError)}</p>` : liveReviews.length ? liveReviews.slice(0, 3).map(ReviewCard).join("") : EmptyState("لا توجد مراجعات بعد", "كن أول من يكتب مراجعة لهذه الرواية.", "/auth/login")}`
    : ReviewForm(book);
  const reviewsAction = isLiveBook ? `<a class="btn ghost" href="#/novels/${book.id}/reviews">كل المراجعات</a>` : "";
  const similarBooks = readerBooks().filter((item) => item.id !== book.id && item.genreSlug === book.genreSlug).slice(0, 3);
  return PageShell(
    "",
    `
      ${NovelHero(book)}
      ...
      <div class="two-column">
        ...
        ${Section("المراجعات", isLiveBook ? "تقييمات ومراجعات القراء." : "المراجعات متاحة للروايات الحقيقية.", reviewsBody, reviewsAction)}
      </div>
      ${Section("روايات مشابهة", "اقتراحات حسب التصنيف.", similarBooks.length ? `<div class="book-grid">${similarBooks.map(BookCard).join("")}</div>` : EmptyState("لا توجد روايات مشابهة حاليا", "ارجع إلى التصفح لاكتشاف تصنيفات أخرى.", "/browse"))}
    `,
  );
}
...
function renderNovelReviews(bookId) {
  ...
  const reviewError = appState.social.reviewErrors.get(book.id);
  const isLiveBook = isUuid(book.id);
  const reviewsContent = isLiveBook
    ? `
      <div class="two-column">
    ...
    `
    : Section("المراجعات", "المراجعات متاحة للروايات الحقيقية.", ReviewForm(book), `<a class="btn ghost" href="#/novels/${book.id}">تفاصيل الرواية</a>`);
  return PageShell(
    "",
    `
...
function renderReader(bookId, chapterId) {
  ...
  if (!liveChapters.length) return PageShell("", EmptyState("لا توجد فصول منشورة", "لا توجد فصول متاحة لهذه الرواية حاليا.", `/novels/${book.id}`));
  const chapter = liveChapters.find((item) => item.id === chapterId);
  if (!chapter) return PageShell("", EmptyState("الفصل غير موجود", "اختر فصلًا من صفحة تفاصيل الرواية.", `/novels/${book.id}`));
  ...
  recordChapterRead(book.id, chapter.id, "opened");
    ...
    `
      <article class="reader-page">
        ...
        <div class="reader-content">
          ...
          ${chapter.coverUrl ? `<img class="reader-chapter-cover" src="${escapeHtml(chapter.coverUrl)}" alt="غلاف ${escapeHtml(chapter.title)}" loading="lazy" />` : ""}
          ${String(chapter.content || "في هذا الفصل التجريبي يبدأ النص هنا. محتوى الفصول الحقيقي سيظهر عند نشر النصوص.")
            .split(/\n{2,}/)
            .filter(Boolean)
...
function renderHistory() {
  if (!appState.auth.ready) return PageShell("/library", LoadingState());
  if (!appState.auth.user) {
    return PageShell("/library", EmptyState("سجل الدخول لفتح سجل القراءة", "سجل القراءة محفوظ لحسابك حتى تتمكن من المتابعة من أي جهاز.", "/auth/login"));
  }
  if (!appState.reading.loaded) {
    loadReadingFeatures();
    return PageShell("/library", LoadingState());
  ...
  return PageShell(
    "/library",
    `
      <div class="page-title"><span class="eyebrow">${dual("السجل", "History")}</span><h1>${dual("سجل القراءة", "Reading History")}</h1><p>آخر الفصول التي فتحتها وحفظت تقدمك فيها.</p></div>
...
function renderProfile() {
  const isSignedIn = Boolean(appState.auth.user);
  const isGuest = appState.auth.isGuest;
  if (isSignedIn && !appState.profileMetrics.loaded) loadProfileMetrics();
  const name = getDisplayName();
  const email = appState.auth.user?.email || "";
  const transientMessage = isSignedIn || isGuest ? appState.authMessage : "";
  const metrics = isSignedIn ? appState.profileMetrics : {
    libraryCount: 0,
    readingCount: 0,
    reviewCount: 0,
    followCount: 0,
  ...
  const description = isSignedIn
    ...
    : isGuest
      ? "تستخدم NOVELFLEX حاليا كضيف. يمكنك تصفح الروايات العامة بدون إنشاء حساب."
...
function renderAuth(mode = "login") {
  ...
  const isSignedIn = Boolean(appState.auth.user);
  const isGuest = appState.auth.isGuest;
  const authNotice = isRegister && appState.authMessage === "تم تسجيل الخروج." ? "" : appState.authMessage;
      ...
      `
        <section class="auth-page">
          <div class="auth-intro">
            ...
            ${authNotice ? `<p class="auth-alert success">${escapeHtml(authNotice)}</p>` : ""}
          </div>
...
function AuthorAccessGate() {
  ...
  if (appState.authorPortal.loadError) {
...
function AuthorTableNotice() {
  ...
  return `<div class="toast">لم يتم العثور على ملف كاتب منفصل، وسيتم الاعتماد على بيانات الحساب الحالية.</div>`;
}

function AuthorCoverUploadNotice() {
  const upload = appState.authorPortal.coverUpload;
  if (!upload?.message) return "";
  return `
    <p class="auth-alert ${upload.active ? "" : "success"}">${escapeHtml(upload.message)}</p>
    ${upload.active ? `<progress value="${upload.progress}" max="100">${upload.progress}%</progress>` : ""}
  `;
}
...
function renderAuthorDashboard() {
  ...
  return AuthorDashboardLayout(
    ...
    `
      ...
      <div class="page-title"><span class="eyebrow">${dual("لوحة الكاتب", "Author Dashboard")}</span><h1>${dual("لوحة الكاتب", "Author Dashboard")}</h1><p>تابع رواياتك ومسوداتك من مكان واحد.</p></div>
      <div class="metric-grid">
      ...
      ${Section("آخر الروايات", "أحدث الروايات المرتبطة بحسابك.", novels.length ? `<div class="list-stack">${novels.slice(0, 4).map(AuthorNovelRow).join("")}</div>` : EmptyState("لا توجد روايات بعد", "أنشئ أول رواية كمسودة من بوابة الكاتب.", "/author/novels/new"), `<a class="btn primary" href="#/author/novels/new">رواية جديدة</a>`)}
    `,
  );
}
...
function NovelForm(mode, book = books[0]) {
  ...
  const defaultStatusAction = isEdit && activeBook.rawStatus === "published" ? "publish" : "draft";
  ...
  return AuthorDashboardLayout(
    ...
    `
      ...
      <form class="editor-form" data-author-novel-form data-novel-mode="${mode}" data-book-id="${escapeHtml(activeBook.id || "")}" data-default-status-action="${defaultStatusAction}">
        ...
        <label>
          <span>رابط غلاف اختياري</span>
          <input name="cover_url" value="${escapeHtml(activeBook.coverUrl || "")}" placeholder="https://..." />
          <input name="existing_cover_url" type="hidden" value="${escapeHtml(activeBook.coverUrl || "")}" />
        </label>
...
function renderChapterManager(bookId, filter = "all") {
  ...
  return AuthorDashboardLayout(
    "/author/novels",
    `
      <div class="page-title"><span class="eyebrow">${dual("إدارة الفصول", "Chapter Manager")}</span><h1>فصول ${escapeHtml(book.title)}</h1><p>أنشئ الفصول ورتبها وعدل حالتها قبل ظهورها للقراء.</p></div>
      ${appState.authorPortal.error ? `<p class="auth-alert error">${escapeHtml(appState.authorPortal.error)}</p>` : ""}
      ${appState.authorPortal.message ? `<div class="toast">${escapeHtml(appState.authorPortal.message)}</div>` : ""}
      <div class="tabs">
      ...
      ${Section("قائمة الفصول", "أنشئ الفصول أو حررها أو غير ترتيبها من هنا.", appState.authorPortal.loadingChapters.has(book.id) ? LoadingState() : chapterError ? EmptyState("تعذر تحميل الفصول", chapterError, "/author/novels") : list.length ? `<div class="list-stack">${list.map((chapter, index) => AuthorChapterRow(book, chapter, index, list.length)).join("")}</div>` : EmptyState("لا توجد فصول", "أضف أول فصل للرواية من هنا.", `/author/novels/${book.id}/chapters/new`), `<a class="btn primary" href="#/author/novels/${book.id}/chapters/new">${dual("إضافة فصل", "Add Chapter")}</a>`)}
    `,
  );
}
...
function renderChapterForm(bookId, chapterId = "") {
  ...
  if (isEdit && !chapter) return AuthorDashboardLayout("/author/novels", EmptyState("الفصل غير موجود", "لم يتم العثور على هذا الفصل ضمن روايتك.", `/author/novels/${book.id}/chapters`));
  return AuthorDashboardLayout(
    "/author/novels",
    `
      <div class="page-title"><span class="eyebrow">${isEdit ? dual("تحرير فصل", "Edit Chapter") : dual("إنشاء فصل", "Create Chapter")}</span><h1>${isEdit ? "تحرير فصل" : "إضافة فصل"} إلى ${escapeHtml(book.title)}</h1><p>اكتب الفصل كنص، أو اربطه بملف، ثم احفظه كمسودة أو انشره للقراء.</p></div>
      ${appState.authorPortal.error ? `<p class="auth-alert error">${escapeHtml(appState.authorPortal.error)}</p>` : ""}
      <form class="editor-form" data-author-chapter-form data-chapter-mode="${isEdit ? "edit" : "create"}" data-book-id="${escapeHtml(book.id)}" data-chapter-id="${escapeHtml(chapter?.id || "")}">
        ...
        <label>
          <span>غلاف الفصل اختياري</span>
          <input name="cover_file" type="file" accept="image/png,image/jpeg,image/webp" />
          <small>النسبة الموصى بها 16:9، مقاس 1600x900، والحد الأقصى 2MB.</small>
        </label>
        <label>
          <span>رابط غلاف الفصل</span>
          <input name="cover_url" value="${escapeHtml(chapter?.coverUrl || "")}" placeholder="https://..." />
          <input name="existing_cover_url" type="hidden" value="${escapeHtml(chapter?.coverUrl || "")}" />
        </label>
        ${chapter?.coverUrl ? `<div class="chapter-cover-preview"><img src="${escapeHtml(chapter.coverUrl)}" alt="غلاف ${escapeHtml(chapter.title)}" /><span>الغلاف الحالي</span></div>` : ""}
        <label class="full"><span>نص الفصل</span><textarea name="content_text" rows="10" placeholder="اكتب نص الفصل هنا، أو أضف مسار ملف الفصل أعلاه.">${escapeHtml(chapter?.content || "")}</textarea></label>
        <div class="form-actions">
...
function renderAnalytics() {
  ...
  const novels = authorNovelsLive();
  const published = authorNovelsLive("published");
  const totalViews = novels.reduce((sum, book) => sum + Number(book.viewsCount || 0), 0);
  const totalLikes = novels.reduce((sum, book) => sum + Number(book.likesCount || 0), 0);
  const totalRatings = novels.reduce((sum, book) => sum + Number(book.ratingsCount || 0), 0);
  const weightedRating = totalRatings
    ? novels.reduce((sum, book) => sum + Number(book.rating || 0) * Number(book.ratingsCount || 0), 0) / totalRatings
    : 0;
  const performanceRows = [...novels].sort((a, b) => Number(b.viewsCount || 0) - Number(a.viewsCount || 0));
  return AuthorDashboardLayout(
    "/author/analytics",
    `
      <div class="page-title"><span class="eyebrow">${dual("التحليلات", "Analytics")}</span><h1>${dual("التحليلات", "Analytics")}</h1><p>مؤشرات مبنية على الروايات المرتبطة بحسابك.</p></div>
      <div class="metric-grid">
        <article><span>رواياتك</span><b>${novels.length}</b></article>
        <article><span>المنشورة</span><b>${published.length}</b></article>
        <article><span>إجمالي القراءات</span><b>${formatCount(totalViews)}</b></article>
        <article><span>التفاعلات</span><b>${formatCount(totalLikes)}</b></article>
        <article><span>متوسط التقييم</span><b>${weightedRating ? weightedRating.toFixed(1) : "لا يوجد"}</b></article>
      </div>
      ${Section("أداء الروايات", totalRatings ? `استنادا إلى ${formatCount(totalRatings)} تقييم.` : "لا توجد تقييمات كافية بعد.", performanceRows.length ? `<div class="list-stack">${performanceRows.map(AuthorAnalyticsRow).join("")}</div>` : EmptyState("لا توجد بيانات تحليلية", "أنشئ رواية وانشرها حتى تبدأ المؤشرات بالظهور.", "/author/novels/new"))}
    `,
  );
}

function AuthorAnalyticsRow(book) {
  const rating = Number(book.ratingsCount || 0) ? Number(book.rating || 0).toFixed(1) : "بدون تقييم";
  return `
    <article class="admin-row">
      ${BookCover(book)}
      <div>
        <h3>${escapeHtml(book.title)}</h3>
        <p>${escapeHtml(book.approval)} · ${formatCount(book.viewsCount)} قراءة · ${formatCount(book.likesCount)} تفاعل · ${escapeHtml(rating)}</p>
      </div>
      <a class="btn secondary" href="#/author/novels/${book.id}/edit">تحرير</a>
      <a class="btn ghost" href="#/novels/${book.id}">معاينة</a>
    </article>`;
...
function renderAcademy() {
  ...
  return AuthorDashboardLayout(
    "/author/academy",
    `
      <div class="page-title"><span class="eyebrow">${dual("الأكاديمية", "Academy")}</span><h1>${dual("أكاديمية الكاتب", "Author Academy")}</h1><p>إرشادات عملية تساعدك على تجهيز الرواية والفصل قبل النشر.</p></div>
      <div class="book-grid">${academyPosts.map(AcademyCard).join("")}</div>
      <div class="two-column">
        ${Section("قائمة ما قبل النشر", "استخدمها قبل نشر أي فصل جديد.", `<div class="list-stack">${academyChecklist.map((item) => `<article class="academy-card"><p>${escapeHtml(item)}</p></article>`).join("")}</div>`, `<a class="btn primary" href="#/author/novels/new">إنشاء رواية</a>`)}
        ${Section("خطوتك التالية", "أكمل العمل من بوابة الكاتب.", `<div class="list-stack"><a class="btn secondary" href="#/author/novels">رواياتي</a><a class="btn secondary" href="#/author/drafts">المسودات</a><a class="btn secondary" href="#/author/analytics">التحليلات</a></div>`)}
      </div>
    `,
  );
}

function AcademyCard(post) {
  return `
    <article class="academy-card">
      <span class="eyebrow">${escapeHtml(post.type)}</span>
      <h3>${escapeHtml(post.title)}</h3>
      <p>${escapeHtml(post.summary)}</p>
      <small>${escapeHtml(post.time)} قراءة</small>
    </article>`;
}
...
function AdminAccessGate(active = "dashboard") {
  ...
  if (!appState.auth.user) {
    return PageShell("/admin", EmptyState("سجل الدخول كمدير", "لوحة الإدارة تحتاج حسابا بصلاحية مدير.", "/auth/login"));
  }
...
function AdminTableErrors() {
  ...
  return `<div class="list-stack">${entries.map(([table, message]) => `<p class="auth-alert error">${escapeHtml(adminTableLabel(table))}: ${escapeHtml(message)}</p>`).join("")}</div>`;
}
...
function renderAdminDashboard() {
  ...
  return AdminLayout(
    "dashboard",
    `
      <div class="page-title"><span class="eyebrow">${dual("الإدارة", "Admin")}</span><h1>${dual("لوحة الإدارة", "Admin Dashboard")}</h1><p>إدارة المستخدمين والمحتوى من مكان واحد.</p></div>
      ${AdminTableErrors()}
      <div class="metric-grid">
...
function renderAdminUsers() {
  ...
  return AdminLayout(
    "users",
    `
      <div class="page-title"><span class="eyebrow">${dual("إدارة المستخدمين", "User Management")}</span><h1>${dual("إدارة المستخدمين", "User Management")}</h1><p>راجع الأدوار وحالة ظهور الحسابات العامة.</p></div>
      ${appState.adminPortal.profiles.length ? `<div class="list-stack">${appState.adminPortal.profiles.map(AdminUserRow).join("")}</div>` : EmptyState("لا توجد ملفات مستخدمين", "لا توجد حسابات متاحة للإدارة حاليا.", "/admin")}
    `,
  );
}
...
function AdminUserRow(profile) {
  return `
    <article class="admin-row">
      ...
      <div><h3>${escapeHtml(profile.display_name || profile.username || profile.id)}</h3><p>${escapeHtml(adminRoleLabel(profile.role || "reader"))} · ${profile.is_public ? "ظاهر" : "مخفي"}</p></div>
      <select data-admin-user-role="${profile.id}">
        ${["reader", "writer", "admin"].map((role) => `<option value="${role}" ${profile.role === role ? "selected" : ""}>${adminRoleLabel(role)}</option>`).join("")}
      </select>
      <button class="btn secondary" data-admin-update-role="${profile.id}" ${appState.adminPortal.saving ? "disabled" : ""}>تحديث</button>
      <button class="btn ghost" data-admin-toggle-public="${profile.id}" data-next-public="${profile.is_public ? "false" : "true"}" ${appState.adminPortal.saving ? "disabled" : ""}>${profile.is_public ? "إخفاء" : "إظهار"}</button>
    </article>`;
}
...
function renderAdminAuthors() {
  ...
  return AdminLayout(
    "authors",
    `
      <div class="page-title"><span class="eyebrow">${dual("إدارة الكتّاب", "Author Management")}</span><h1>${dual("إدارة الكتّاب", "Author Management")}</h1><p>راجع ملفات الكتّاب وحالة اعتمادهم.</p></div>
      ${error ? `<p class="auth-alert error">${escapeHtml(error)}</p>` : ""}
      ${appState.adminPortal.authors.length ? `<div class="list-stack">${appState.adminPortal.authors.map(AdminAuthorRow).join("")}</div>` : EmptyState("لا توجد بيانات كتّاب", "لا توجد ملفات كتّاب متاحة حاليا.", "/admin")}
    `,
  );
}
...
function AdminAuthorRow(author) {
  return `
    <article class="admin-row">
      <div>
        <h3>${escapeHtml(author.pen_name || author.profile?.display_name || author.profile?.username || author.id)}</h3>
        <p>${escapeHtml(author.bio_ar || author.bio_en || (author.is_approved ? "كاتب معتمد" : "كاتب بانتظار الاعتماد"))}</p>
      </div>
    </article>`;
}

function renderAdminNovels({ moderation = false } = {}) {
  const gate = AdminAccessGate("novels");
  if (gate) return gate;
  const books = moderation ? appState.adminPortal.books.filter((book) => ["in_review", "rejected"].includes(book.rawStatus)) : appState.adminPortal.books;
  return AdminLayout(
    moderation ? "content" : "novels",
    `
      <div class="page-title"><span class="eyebrow">${moderation ? dual("مراجعة المحتوى", "Content Moderation") : dual("إدارة الروايات", "Novel Management")}</span><h1>${moderation ? "مراجعة المحتوى" : dual("إدارة الروايات", "Novel Management")}</h1><p>${moderation ? "راجع الروايات التي تحتاج قرارا تحريريا قبل النشر." : "راجع حالة الروايات وحدد ما يظهر للقراء."}</p></div>
      ${appState.adminPortal.tableErrors.books ? `<p class="auth-alert error">${escapeHtml(appState.adminPortal.tableErrors.books)}</p>` : ""}
      ${books.length ? `<div class="list-stack">${books.map(AdminBookRow).join("")}</div>` : EmptyState(moderation ? "لا توجد روايات بانتظار المراجعة" : "لا توجد روايات", moderation ? "كل الروايات الحالية لا تحتاج مراجعة الآن." : "لا توجد روايات متاحة للإدارة حاليا.", "/admin")}
    `,
  );
}
  ...
  return `
    <article class="admin-row">
      ...
      <select data-admin-book-status="${book.id}">
        ${["draft", "in_review", "published", "rejected", "archived"].map((status) => `<option value="${status}" ${book.rawStatus === status ? "selected" : ""}>${adminBookStatusLabel(status)}</option>`).join("")}
      </select>
      <button class="btn secondary" data-admin-update-book="${book.id}" ${appState.adminPortal.saving ? "disabled" : ""}>حفظ الحالة</button>
    </article>`;
}
...
function renderAdminReviews() {
  ...
  return AdminLayout(
    "reviews",
    `
      <div class="page-title"><span class="eyebrow">${dual("مراجعة التقييمات", "Review Moderation")}</span><h1>${dual("مراجعة التقييمات", "Review Moderation")}</h1><p>راجع تقييمات القراء الحديثة واستخدم البلاغات لاتخاذ إجراء عند الحاجة.</p></div>
      ${appState.adminPortal.tableErrors.ratings ? `<p class="auth-alert error">${escapeHtml(appState.adminPortal.tableErrors.ratings)}</p>` : ""}
      ${appState.adminPortal.reviews.length ? `<div class="list-stack">${appState.adminPortal.reviews.map(AdminReviewRow).join("")}</div>` : EmptyState("لا توجد مراجعات", "لا توجد تقييمات من القراء حاليا.", "/admin")}
    `,
  );
}
...
function AdminReviewRow(review) {
  ...
  return `
    <article class="admin-row">
      <div>
        <h3>${rating ? `${escapeHtml(String(rating))} من 5` : "تقييم بدون درجة"}</h3>
        <p>${escapeHtml(reviewText || "لا يوجد نص مرفق مع هذا التقييم.")}</p>
        <p>الرواية ${escapeHtml(shortAdminId(review.book_id))} · المستخدم ${escapeHtml(shortAdminId(review.user_id))} · ${escapeHtml(adminDate(review.created_at))}</p>
      </div>
      <a class="btn ghost" href="#/admin/reports">البلاغات</a>
    </article>`;
}
...
function renderAdminReports() {
  ...
  return AdminLayout(
    "reports",
    `
      <div class="page-title"><span class="eyebrow">${dual("طابور البلاغات", "Reports Queue")}</span><h1>${dual("طابور البلاغات", "Reports Queue")}</h1><p>راجع البلاغات المفتوحة وغيّر حالتها بعد اتخاذ القرار المناسب.</p></div>
      ${appState.adminPortal.tableErrors.reports ? `<p class="auth-alert error">${escapeHtml(appState.adminPortal.tableErrors.reports)}</p>` : ""}
      ${appState.adminPortal.reports.length ? `<div class="list-stack">${appState.adminPortal.reports.map(AdminReportRow).join("")}</div>` : EmptyState("لا توجد بلاغات", "لا توجد بلاغات تحتاج مراجعة حاليا.", "/admin")}
    `,
  );
}
...
function AdminReportRow(report) {
  ...
  const details = adminReportDetails(report);
  return `
    <article class="admin-row">
      <div>
        <h3>${escapeHtml(report.reason || report.type || "بلاغ جديد")}</h3>
        <p>${escapeHtml(adminReportStatusLabel(status))} · ${escapeHtml(adminReportTarget(report))} · ${escapeHtml(adminDate(report.created_at))}</p>
        <p>المبلّغ ${escapeHtml(shortAdminId(report.reporter_id || report.user_id))}${report.reported_user_id ? ` · ضد ${escapeHtml(shortAdminId(report.reported_user_id))}` : ""}</p>
        ${details ? `<p>${escapeHtml(details)}</p>` : ""}
      </div>
      <select data-admin-report-status="${report.id}">
        ${["open", "reviewing", "resolved", "dismissed"].map((option) => `<option value="${option}" ${status === option ? "selected" : ""}>${adminReportStatusLabel(option)}</option>`).join("")}
      </select>
      <button class="btn secondary" data-admin-update-report="${report.id}" ${appState.adminPortal.saving ? "disabled" : ""}>حفظ</button>
    </article>`;
}
...
function renderAdminCategories() {
  ...
  return AdminLayout(
    "categories",
    `
      <div class="page-title"><span class="eyebrow">${dual("التصنيفات", "Categories")}</span><h1>${dual("التصنيفات", "Categories")}</h1><p>رتّب التصنيفات وحدد ما يظهر في التصفح والبحث.</p></div>
      ...
      ${appState.adminPortal.categories.length ? `<div class="list-stack">${appState.adminPortal.categories.map(AdminCategoryRow).join("")}</div>` : EmptyState("لا توجد تصنيفات", "لا توجد تصنيفات متاحة للإدارة حاليا.", "/admin")}
    `,
  );
}
    ...
    <article class="admin-row">
      <form class="editor-form full" data-admin-category-form="${category.id}">
        <div class="full">
          ...
          <p>${escapeHtml(category.slug)} · ${category.isActive ? "ظاهر في التصفح" : "مخفي عن التصفح"}</p>
        </div>
        <label><span>الاسم</span><input name="name_ar" value="${escapeHtml(category.name)}" required /></label>
        <label><span>${dual("المعرّف", "Slug")}</span><input name="slug" value="${escapeHtml(category.slug)}" required /></label>
        ...
        <div class="form-actions"><button class="btn secondary" type="submit" ${appState.adminPortal.saving ? "disabled" : ""}>حفظ التصنيف</button></div>
      </form>
    </article>`;
}
...
function route() {
  ...
  else if (path === "/admin/content-moderation") html = renderAdminNovels({ moderation: true });
...
function bindInteractions() {
  ...
  document.querySelectorAll('[data-author-novel-form] input[name="cover_file"]').forEach((input) => {
    input.addEventListener("change", () => {
      const file = input.files?.[0];
      if (!file) {
        setAuthorCoverUploadStatus("", 0, false);
